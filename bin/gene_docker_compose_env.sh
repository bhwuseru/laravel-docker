...
[ ! -f "$DOCKER_ENVFILE_PATH" ] && {
  ...
  echo "DB_TEST_PORT=${DB_TEST_PORT}"
  echo "USER=${USER}"
...
echo "CREATE DATABASE IF NOT EXISTS ${DB_DATABASE}_test CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;GRANT ALL PRIVILEGES ON ${DB_DATABASE}_test.* TO 'user'@'%' IDENTIFIED BY '${DB_PASSWORD}';FLUSH PRIVILEGES;" > "${PROJECT_NAME_DIR_PATH}/db/init/init.sql"

# テスト用SQLファイル作成
# echo "CREATE DATABASE IF NOT EXISTS ${DB_DATABASE} CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;" > "${PROJECT_NAME_DIR_PATH}/db-test/init/init.sql"
...
# echo "GRANT ALL ON ${DB_DATABASE}_testing.* TO '${DB_USER}'@'%';" >> "$DB_INIT_SQLFILE_PATH"
